...
module.exports = {
    // phân trang ...
    getHomeSearchGamePhanTrang: (req, res) => { 
        if (req.query.page) {
            return res.redirect(`/search-qly-game?search_ben_admin=${req.query.search_ben_admin}&page=${req.query.page}`)
        }
        res.redirect(`/search-qly-game`)
    },

    // home quản lý nước hoa
    getHomeSearchGame: async (req, res) => {
        ...
        res.render("AdminQL/TrangQLAdmin/QuanLySanPham/QuanLySPGame/getSearchGame.ejs", {
            tk, logged, activee,
            rootPath: '/', 
